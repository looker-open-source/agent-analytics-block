  ...
  derived_table: {
    sql:
      SELECT
        ...
        CAST(JSON_VALUE(attributes, '$.usage_metadata.prompt_token_count') AS INT64) AS usage_prompt_tokens,
        CAST(JSON_VALUE(attributes, '$.usage_metadata.candidates_token_count') AS INT64) AS usage_completion_tokens,
        CAST(JSON_VALUE(attributes, '$.usage_metadata.total_token_count') AS INT64) AS usage_total_tokens,
        CAST(JSON_VALUE(attributes, '$.usage_metadata.cached_content_token_count') AS INT64) AS usage_cached_tokens,
        SAFE_DIVIDE(CAST(JSON_VALUE(attributes, '$.usage_metadata.cached_content_token_count') AS INT64),CAST(JSON_VALUE(attributes, '$.usage_metadata.prompt_token_count') AS INT64)) AS context_cache_hit_rate,
